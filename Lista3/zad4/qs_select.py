...
def qs_select(n, array, p, r):
    if r-p+1 == n:
        ...
        swap_counter = 0
    if p<r:
        pivot = select(array[p:r+1], (r-p+1)//2)
        j = partition(array, p, r, pivot)
        qs_select(n, array, p, j-1)
        qs_select(n, array, j+1, r)
    if r-p+1 == n:
...
def qs_select_stat(n, array, p, r, filename):
    if r-p+1 == n:
        global alg_time, comp_counter, swap_counter
        alg_time = time.time()
        comp_counter = 0
        swap_counter = 0
    if p<r:
        pivot = select_stat(array[p:r+1], (r-p+1)//2)
        j = partition_stat(array, p, r, pivot)
        qs_select_stat(n, array, p, j-1, filename)
        qs_select_stat(n, array, j+1, r, filename)
    if r-p+1 == n:
        alg_time = time.time() - alg_time
